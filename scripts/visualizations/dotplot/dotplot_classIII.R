...
# Filter for `mhc_class_region == 3`
pacbio_data <- pacbio_data %>%
  filter(mhc_class_region == 3)

promethion_data <- promethion_data %>%
  filter(mhc_class_region == 3)
...
ggsave(filename = "dotplot_classIII.pdf", plot = combined_plot, width=169, units = "mm")
ggsave(filename = "dotplot_classIII.png", plot = combined_plot, width=169, units = "mm")
